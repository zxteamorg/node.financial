...
testCases.forEach(function (testCase) {
	...
	backends.forEach(function (backend: Settings.Backend) {
		...
		describe(`max should be ${left} vs ${right} = ${expectedResult}`, function () {
			...
			it("financial.max(left: FinancialLike, right: FinancialLike): FinancialLike", function () {
				const friendlyLeft: FinancialLike = financial.parse(left);
				const friendlyRight: FinancialLike = financial.parse(right);
				const result: FinancialLike = financial.max(friendlyLeft, friendlyRight);
			...
			it("value.add(right: FinancialLike): FinancialLike", function () {
				const friendlyLeft: FinancialLike = financial.parse(left);
				const friendlyRight: FinancialLike = financial.parse(right);
				const result: FinancialLike = friendlyLeft.max(friendlyRight);
				assert.equal(result.toString(), expectedResult);
			});
		});
	});
